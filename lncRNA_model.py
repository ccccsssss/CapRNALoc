...
class PrimaryCapsuleChannelAttention(nn.Module):
    ...
    def forward(self, primary_capsules):
        ...
        out = primary_capsules * channel_weight.unsqueeze(1)


        squared_norm = (out ** 2).sum(-1, keepdim=True)
        out = squared_norm * out / ((1. + squared_norm) * torch.sqrt(squared_norm))
...
class CapsNet(nn.Module):
    ...
    def forward(self, data):
        ...
        channel_output = self.channel_attention(primary_output)
        digit_output = self.digit_capsules(channel_output)
        output = digit_output
